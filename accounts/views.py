...
def teacherlogin(request):
    if request.method == 'POST':
        ...
        teacher = Teacher.objects.raw(
            'SELECT * FROM accounts_teacher WHERE teacher_name =%s AND password=%s', [teachername, teacherpassword])
...
def handlelogin(request):
    if request.method == 'POST':
        ...
        student = Student.objects.raw(
            'SELECT * FROM accounts_student WHERE username =%s AND password=%s', [lusername, lpassword])
...
def openteacher(request, teacherid):
    myteacher = Teacher.objects.raw(
        'SELECT * FROM accounts_teacher WHERE id = %s', [teacherid])
    # course = Course.objects.get(pk=myteacher.course.pk)
    course = Course.objects.raw(
        'SELECT * FROM accounts_course WHERE course_id = %s', [myteacher.course__pk])[0]
...
def task(request, studentid, taskid):
    ...
    task = Todo.objects.raw(
        'SELECT * FROM accounts_todo WHERE id = %s', [taskid])[0]
    task.delete()
...
def addtodo(request, studentid):
    todo_obj = Todo()
    todo_obj.student = Student.objects.raw(
        'SELECT * FROM accounts_student WHERE student_id=%s', [studentid])[0]
...
def openstudent(request, studentid):
    ...
    for cour in courses:
        ...
        myteacher = Teacher.objects.raw(
            'SELECT * FROM accounts_teacher WHERE course_id=%s', [cour.course_id])[0]
        ...
        for lect in lectures:
            ...
            #     w_lect__pk=lect.pk)
            mywatch_time = Watch_time.objects.raw(
                'SELECT * FROM accounts_watch_time WHERE student_id=%s AND w_lect_id=%s', [studentid, lect.lec_id])
    ...
    tasks = Todo.objects.raw(
        'SELECT * FROM accounts_todo WHERE student_id=%s', [studentid])
    context = {
        'student': student,
        'myfile': myfile,
        'mylist': mylist,
...
def openLectlistfromstudent(request, studentid, courseid):
    ...
    course = Course.objects.raw(
        'SELECT * FROM accounts_course WHERE course_id=%s', [courseid])[0]
    ...
    ##
    mywatch_time = Watch_time.objects.raw(
        'SELECT * FROM accounts_watch_time where student_id=%s', [studentid])
...
def openLecturefromstudent(request, studentid, courseid, lectid):
    if request.method == 'POST':
        ...
        #     w_lect__pk=lectid)
        mywatch_time = Watch_time.objects.raw(
            'SELECT * FROM accounts_watch_time WHERE student_id=%s AND w_lect_id=%s', [studentid, lectid])
        for watch in mywatch_time:
            watch.Rating = rating
            watch.save()
        ...
        # myteacher = Teacher.objects.get(course__pk=courseid)
        myteacher = Teacher.objects.raw(
            'SELECT * FROM accounts_teacher WHERE course_id = %s', [courseid])[0]
        # lecture = Lecture.objects.filter(teacher__pk=myteacher.pk).get(pk=lectid)
        lecture = Lecture.objects.raw(
            'SELECT * FROM accounts_lecture WHERE teacher_id = %s AND lec_id = %s', [myteacher.pk, lectid])[0]
        # course = Course.objects.get(pk=courseid)
        course = Course.objects.raw(
            'SELECT * FROM accounts_course WHERE course_id=%s', [courseid])[0]
        # student = Student.objects.get(pk=studentid)
        ...
        lecture = Lecture.objects.raw(
            'SELECT * FROM accounts_lecture WHERE lec_id=%s', [lectid])[0]
...
def viewallcourses(request):
    ...
    for course in courses:
        ...
        myteacher = Teacher.objects.raw(
            'SELECT * FROM accounts_teacher WHERE course_id=%s', [course.course_id])[0]
        # lectures = Lecture.objects.filter(teacher__pk=myteacher.pk)
        lectures = Lecture.objects.raw(
            'SELECT * FROM accounts_lecture WHERE teacher_id=%s', [myteacher.teacher_id])
        lect_counter = 0
        ...
        for lect in lectures:
            ...
            mywatch_time = Watch_time.objects.raw(
                'SELECT * FROM accounts_watch_time WHERE w_lect_id=%s', [lect.lec_id])
            watch_counter = 0
            for watch in mywatch_time:
                if watch.completed == True:
                    watch_counter += 1
...
def addstudent(request):
    if request.method == 'POST':
        ...
        try:
            ...
            myteacher = Teacher.objects.filter(course=courses)
            lectures = Lecture.objects.filter(teacher=myteacher.all())
            print('\n\n\n\n\n\n\n\n')
            print(lectures)
            print('\n\n\n\n\n\n\n\n')

        except Exception as e:
            print("1 An exception occurred: ", e)
...
def newcourses(request):
    all_courses = Course.objects.raw("SELECT * FROM course_view")
    context = {
        'courses': all_courses,
    }
